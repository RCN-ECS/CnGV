...
packages_needed <- c("plyr","tidyverse","lme4","gridExtra","ggplot2")
...
Diff_GxE <- list(
  ...
  "intercept_G1" = 0,
  "slope_G1" = c(-2,-1,-0.25), #seq(from = -1, to = 1, by = 0.5),
  "intercept_G2" = c(0), #seq(from = -5, to = 5, by = 2),
  "slope_G2" = c(2,1,0.25), #seq(from = -1, to = 1, by = 0.1),
  "sd" = c(0.5,1,1.5), #seq(from = 0, to = 1, by = 0.5),
...
Diff_means <- list(
  "type" = c("cogv","cngv","pure_GxE"),
  "intercept_G1" = 0,
  "slope_G1" = c(2,1),#seq(from = -1, to = 1, by = 0.5),
  "intercept_G2" = c(4,2),#seq(from = -5, to = 5, by = 2),
  "slope_G2" = c(-2,1),#seq(from = -1, to = 1, by = 0.1),
  "sd" = c(0.5,1),#seq(from = 0, to = 1, by = 0.5),
  "sample_size" = c(5,7,9),#seq(from = 5, to = 10, by = 2),
  "env_num" = 2,#c(2,5,10), 
  "env" = NA,
  "G1_env" = NA, 
  "G2_env" = NA,
...
# Sanity check
for(i in 1:length(unique(new_data1$index))){
  ind = unique(new_data1$index)[i]
  plotdat = filter(new_data1, index == ind)
...
Cov_matrix_sim <- function(genphenenv_df){
  ...
  for(m in 1:length(unique(cov_temp$native_env))){
    ...
    model_specs = data.frame("Index" = unique(genphenenv_df$index),
                             "G1_slope" = unique(genphenenv_df$slope[genphenenv_df$gen == "G1"]),
                             "G2_slope" = unique(genphenenv_df$slope[genphenenv_df$gen == "G2"]),
                             "slope_diff" = unique(genphenenv_df$slope_diff),
                             "error" = unique(genphenenv_df$stdev),
                             "lm_result" = lm_result,
                             "GxE_pval" = GxE_pval,
...
for(i in 1:length(unique(new_data$index))){
  ...
  filter_dat = filter(new_data, index == ind)
test_dat_temp <- Cov_matrix_sim(filter_dat)
test_dat <- rbind(test_dat,test_dat_temp[[3]])
}
print(ggplot(test_dat, aes(x = slopeset, y = as.numeric(GxE_R2), group=error,fill = error)) + 
    geom_bar(stat = "identity",position="dodge")+theme_classic())+
    labs(x = "G1 and G2 Slopes")
...
results = Categorical_data(faske)
