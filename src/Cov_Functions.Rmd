...
Diff_GxE <- list(
  ...
  "intercept_G2" = 4, #seq(from = -5, to = 5, by = 2),
...
Diff_means <- list(
  ...
  "slope_G2" = c(-1,1),#seq(from = -1, to = 1, by = 0.1),
  "sd" = c(0.5,1),#seq(from = 0, to = 1, by = 0.5),
...
for(i in 1:length(unique(new_data$index))){
  index = unique(new_data$index)[i]
  plotdat = new_data[new_data$index== index,]
  plot_temp = ggplot(plotdat, aes(x = env, y = phen, group = gen, fill = gen, colour = gen)) + 
  geom_smooth(method = "gam") + theme_classic()
  print(plot_temp)##Why do some of these have the same intercept? They shouldn't. 
}
