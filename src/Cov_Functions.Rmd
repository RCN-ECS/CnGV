...
Cov_matrix_sim <- function(genphenenv_df){
  ...
  cov_temp <- data.frame(gen = rep(unique(genphenenv_df$gen),
                         ...
                         env_corrected = seq(from = min(genphenenv_df$env_corrected), to = (max(genphenenv_df$env_corrected)+0.1),by=0.1))
...
Cov_matrix_continuous <- function(genphenenv_df){
  ...
  cov_temp$env_corrected <- round(cov_temp$env_corrected, digits = 1)
  cov_temp$native_env_corrected <- round(genphenenv_df$native_env_corrected[match(cov_temp$gen_factor,genphenenv_df$gen_factor)],digits =1)
...
falc_dat_cat = read.csv("~/Documents/GitHub/CnGV/data/Falconer_template_categoricalenv.csv") # with  categorical environental data for testing
gordon = read.csv("~/Downloads/4DataCollection_Template_studyID_654.csv") # with  categorical environental data for testing
...
# Raw data
ggplot(faske,aes(x = exp_env_cont, y = phen_data, group = gen_factor, fill = gen_factor, colour = gen_factor)) + 
  geom_smooth(method = "gam") + theme_classic()

# new data after standardizing
ggplot(newdat,aes(x = env_corrected, y = phen_corrected, group = gen_factor, fill = gen_factor, colour = gen_factor)) + 
  geom_smooth(method = "gam") + theme_classic()
...
bootstap_categorical(genphenenv_df)
...
permutation_categorical <- function(genphenenv_df){
  ...
  perm_dat = data.frame()
   ...
   for (j in 1:nlevels(genphenenv_df$gen_factor)){
      ...
      null_temp <- sample(cond_E$phen_data, size=nrow(cond_E), replace=FALSE)
      
      perm_dat_temp <- data.frame("gen_factor" = cond_E$gen_factor,
                                  "exp_env_factor" = cond_E$exp_env_factor,
                                  "Native_env_cat" = cond_E$Native_env_cat,
                                  "nat_env_factor" = cond_E$nat_env_factor,
                                  "phen_data" = null_temp)
...
Categorical_data <- function(genphenenv_df){
  ...
  ## Covariance estimate based on real data
  ...
  ## Is covariance outside null distribution? 
  alpha = 0.05
  ...
  # P-value = outside distribution (p = 0.0). Greater than 0.5, 1-value. Less than, keep. Probability of observing by chance
  p_value = (rank(c(obs,new_dat))[1])/(iterations+1) #If incorrect, see OLD CODE
...
results = Continuous_data(lampei)
