...
param_list <- list(
  ...
  interaction= c(1)) # this sd determines the amount of GxE)

# Table of parameters
table_fun <- function(param_list){
  
  # Basic parameters
  param_temp <- expand.grid("reps" = param_list$reps,
                            "delta_env" = param_list$delta_env,
  ...
  index <- rep(1:n_combo, each = param_list$reps)
  
  # Final data frame
  param_table <- data.frame("row" = row,
                            "index" = index,
                            param_temp)
