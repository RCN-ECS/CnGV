...
input <- list(
  ...
  "slope_G2" = 0.5,#seq(from = -1, to = 1, by = 0.1),
            ...
            for(g in 1:length(sd)){
              for(h in 1:length(sample_size)){
                  ...
                  for(l in 1:length(unique(cov_temp$iteration))){
                    ...
                    for(m in 1:length(unique(iter_dat2$gen))){
                      E = unique(iter_dat2$gen)[m]
                      E_temp = iter_dat2[which(iter_dat2$gen == E),]
                  ...
                  ## Next, come up with way to do permutation 
                  # Not sure where to sample from? definitely from dat_temp, but each iteration or full? 
