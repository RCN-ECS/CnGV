...
the_ring <- function(input_df){
  ...
  for(a in 1:env_num){ # Rethink this... unecessarily replicating data.
    for(b in 1:length(unique(input_df$type))){
    
    #env_num_temp = env_num[a] # Change back to this when I run more than one
    env_num_temp = env_num
    type_temp = type[b]
  
    # Set environmental parameters
    #if(env_num_temp == 2){env = c(1,5) # Change back when I run more than one
    #}else if (env_num_temp == 5){env = c(1,2,3,4,5)
    #}else{env = seq(from = 0.5, to = 5, by = 0.5)}
    
    # Match genotypes to phenotypes
    if(type_temp == "cogv"){((G1_env= 1) & (G2_env = 5)) 
      }else{ ((G1_env= 5) & (G2_env = 1))}
  
  
    #for(c in 1:length(env)){ # may not need this? 
            ...
            for(g in 1:length(sd)){
              for(h in 1:length(sample_size)){
                  ...
                  for(i in 1:100){
                    ...
                    temp1 = data.frame("iteration" = iteration,
                                       "phen" = phen1,
                                       "gen" = rep("G1", length(env_num_temp*reps[h])),
                                       "env" = rep(env, reps[h]),
                                       "G_env" = rep(G1_env,length(env_num_temp*reps[h])),
                    ...
                    temp2 = data.frame("iteration" = iteration,
                                       "phen" = phen2,
                                       "gen" = rep("G2", length(env_num_temp*reps[h])),
                                       "env" = rep(env, reps[h]),
                                       "G_env" = rep(G2_env,length(env_num_temp*reps[h])),
                                       "sample_size" = rep(reps[h], length(env_num_temp*reps[h])))
                    dat_temp. = rbind(temp1, temp2)
                    dat_temp = rbind(dat_temp, dat_temp.)
                  }
                  
 ## Check - whole dataset or each iteration separately?
                  ...
                  # Model Comparison
                  test_temp_a = lmer(phen_corrected ~ env + gen + (1|iteration), data = dat_temp)
                  test_temp_b = lmer(phen_corrected ~ env * gen + (1|iteration), data = dat_temp)
                  result = anova(test_temp_a,test_temp_b)
                  
                  # Model Outputs
                  if(result[[2,8]] > 0.05){
                    test_temp = lmer(phen_corrected ~ env + gen + (1|iteration), data = dat_temp)
                    con_ints = confint(test_temp)
                    lm_type = "No_GxE"
                    slope_G1_lm = mean(coef(test_temp)[[1]][,2])
                    slope_G2_lm = mean(coef(test_temp)[[1]][,2])
                    intercept_G1_lm = mean(coef(test_temp)[[1]][,1])
                    intercept_G2_lm = (mean(coef(test_temp)[[1]][,1])+mean(coef(test_temp)[[1]][,3]))
                    slope_GxE_lm = 0
                    lowCI_G1_int_lm = con_ints[3]
                    highCI_G1_int_lm =  con_ints[8]
                    lowCI_G2_int_lm =  con_ints[5]
                    highCI_G2_int_lm =  con_ints[10]
                    lowCI_G1_slope_lm =  con_ints[4]
                    highCI_G1_slope_lm =  con_ints[9]
                    lowCI_G2_slope_lm =  con_ints[4]
                    highCI_G2_slope_lm =  con_ints[9]
                    ...
                    lm_type = "GxE"
                    slope_G1_lm = mean(coef(test_temp)[[1]][,2])
                    slope_G2_lm = (mean(coef(test_temp)[[1]][,2]) + mean(coef(test_temp)[[1]][,4]))
                    intercept_G1_lm = mean(coef(test_temp)[[1]][,1])
                    intercept_G2_lm = (mean(coef(test_temp)[[1]][,1]) + mean(coef(test_temp)[[1]][,3]))
                    slope_GxE_lm = mean(coef(test_temp)[[1]][,4])
                    lowCI_G1_int_lm = con_ints[3]
                    highCI_G1_int_lm = con_ints[9]
                    lowCI_G2_int_lm = con_ints[5]
                    highCI_G2_int_lm = con_ints[11]
                    lowCI_G1_slope_lm = con_ints[4]
                    highCI_G1_slope_lm = con_ints[10]
                    lowCI_G2_slope_lm = con_ints[6]
                    highCI_G2_slope_lm = con_ints[12]
                    E_R2 = anova(test_temp)[1,2]/sum(anova(test_temp)[2])
                    G_R2 = anova(test_temp)[2,2]/sum(anova(test_temp)[2])
                    GxE_R2 = anova(test_temp)[3,2]/sum(anova(test_temp)[2])
                    }
                  ...
                  if(result[[2,8]] > 0.05){
                    ...
                    cov_temp = data.frame()
                    for(k in 1:length(unique(dat_temp$iteration))){
                      
                      iter = unique(dat_temp$iteration)[k]
                      iter_dat = filter(dat_temp, iteration == iter)
                      test_temp = lm(phen_corrected ~ env + gen, data = iter_dat)
                  
                      cov_temp. <- data.frame(gen = rep(unique(iter_dat$gen),each = 5),
                                              env = as.integer(seq(from = min(iter_dat$env), to = max(iter_dat$env),by=1)))
                      cov_temp.$phen_predicted = predict(test_temp,cov_temp.)
                      cov_temp.$iteration = rep(iter,nrow(cov_temp.))
                      cov_temp = rbind(cov_temp,cov_temp.)
                    }
                    }else{
                  ...
                  # Assign native environments
                  cov_temp$G_env = NA
                  for(z in 1:nrow(cov_temp)){
                    if(cov_temp$gen[z] == "G1"){cov_temp$G_env[z] = G1_env}else{{cov_temp$G_env[z] = G2_env}}
                  }
                  
                  # Covariance Emeans and Gmeans
                  Cov_matrix = data.frame()
                  for(l in 1:length(unique(cov_temp$iteration))){
                    iter = unique(cov_temp$iteration)[l]
                    iter_dat2 = filter(cov_temp, iteration == iter)
                  
                    # G_means
                    E_hat = mean(env)
                    G_means = filter(iter_dat2, env == E_hat)
                  
                    # E_means
                    E_mean_temp = data.frame()
                    
                    for(m in 1:length(unique(iter_dat2$G_env))){
                      E = unique(iter_dat2$G_env)[m]
                      E_temp = iter_dat2[which(iter_dat2$env == E),]
                      E_mean = mean(E_temp$phen_predicted)
                      E_mean_temp1 = data.frame("E_mean" = E_mean,
                                                #"gen" = unique(E_temp$gen),
                                                "iteration" = unique(E_temp$iteration))
                    ...
                    Cov_matrix. = data.frame("gen" = unique(cov_temp$gen),
                                             ...
                                             "G_means" = G_means$phen_predicted,
                                             "E_means" = E_mean_temp[,1],
                                             "iteration" = rep(iter,2))
                    Cov_matrix = rbind(Cov_matrix,Cov_matrix.)
                  }
                  
                  # Covariance matrix
                  cov_dat = data.frame()
                  for(n in 1:length(unique(Cov_matrix$iteration))){
                    iter = unique(Cov_matrix$iteration)[n]
                    iter_dat3 = filter(Cov_matrix, iteration == iter)
                  
                    cov_dat. = data.frame("cov_est" = cov(iter_dat3$G_means,iter_dat3$E_means)) 
                    cov_dat = rbind(cov_dat, cov_dat.)
                  ...
                  # Shuffle matrices for Permutation
                  temp_nullmat = data.frame()
                  for(o in 1:length(unique(cov_temp$iteration))){
                    iter = unique(cov_temp$iteration)[o]
                    temp_null = filter(cov_temp, iteration == iter)
                    
                    null_temp = sample(temp_null[,"phen_predicted"], size = nrow(temp_null), replace=FALSE) 
                    
                    temp_nullmat. <- data.frame("gen" = temp_null[,"gen"],
                                                "env" = temp_null[,"env"],
                                                "phen_predicted" = null_temp,
                                                "iteration" = temp_null[,"iteration"],
                                                "G_env" = temp_null[,"G_env"])
                    temp_nullmat = rbind(temp_nullmat, temp_nullmat.)
                  }
                  
                  # Re-estimate G and E means on reshuffled data
                  Perm_matrix = data.frame()
                  for(p in 1:length(unique(temp_nullmat$iteration))){
                    iter = unique(temp_nullmat$iteration)[p]
                    iter_dat3 = filter(temp_nullmat, iteration == iter)
                  
                    # G_means
                    E_hat = mean(env)
                    G_means = filter(iter_dat3, env == E_hat)
                  
                    # E_means
                    E_mean_temp = data.frame()
                    
                    for(q in 1:length(unique(iter_dat3$G_env))){
                      E = unique(iter_dat3$G_env)[q]
                      E_temp = iter_dat3[which(iter_dat3$env == E),]
                      E_mean = mean(E_temp$phen_predicted)
                      E_mean_temp1 = data.frame("E_mean" = E_mean,
                                                #"gen" = unique(E_temp$gen),
                                                "iteration" = unique(E_temp$iteration))
                    ...
                    Perm_matrix. = data.frame("gen" = unique(temp_nullmat$gen),
                                              "G_means" = G_means$phen_predicted,
                                              "E_means" = E_mean_temp[,1],
                                              "iteration" = rep(iter,2))
                    Perm_matrix = rbind(Perm_matrix,Perm_matrix.)
                  }
                  
                  # Calculate permutation covariance estimates
                  perm_dat = data.frame()
                  for(r in 1:length(unique(Perm_matrix$iteration))){
                    iter = unique(Perm_matrix$iteration)[r]
                    iter_dat4 = filter(Perm_matrix, iteration == iter)
                  
                    perm_dat. = data.frame("cov_est" = cov(iter_dat4$G_means,iter_dat4$E_means))
                    perm_dat = rbind(perm_dat, perm_dat.)
                    
                  }
                  
                  # Estimate p-values
                  alpha = 0.05
                  nulldist_lwrCI = quantile(perm_dat[,1], alpha/2)
                  nulldist_uprCI = quantile(perm_dat[,1], 1-alpha/2)
                  
                  is.sig = NA
                  if( (cov_avg < nulldist_lwrCI[[1]]) | (cov_avg > nulldist_uprCI[[1]]) ) {is.sig = "yes"}else{is.sig = "no"}

                                    
                  perm_val = (rank(c(cov_avg,perm_dat[,1]))[1])/(length(perm_dat[,1])+1) #Check this
                  (pvalue <- perm_val/(length(unique(Perm_matrix$iteration)) + 1))
                  
                  covariance_output_df. <- data.frame("lm_type" = lm_type,
                                           "type" = type_temp,
                                           "sd" = sd[g],
                                           "slope_G1" = slope_G1[e],
                                           "slope_G2" = slope_G2[f],
                                           ...
                                           "cov_lwrCI" = cov_CI[[1]],
                                           "cov_uprCI" = cov_CI[[2]],
                                           "nulldist_lwrCI" = nulldist_lwrCI[[1]],
                                           "nulldist_uprCI" = nulldist_uprCI[[1]],
                                           "is.sig" = is.sig,
                                           "cov_pval" = pvalue) 
                  covariance_output_df = rbind(covariance_output_df, covariance_output_df.)
