...
Gradient_level_fun  <- function(z){
  ...
  # G_means
  G_means = filter(temp_dat, env == E_hat)
  
  # E_means
  E_means1 = data.frame()
  for(j in 1:length(E_means)){
    env_dat = E_means[j]
    E_dat = filter(temp_dat, env == env_dat)
...
shuffle_fun <- function(raw_dat){ 
  ...
  for(n in 1:length(raw_dat2)){
      phen_temp = sample(raw_dat2[[n]][,"phen_corrected"], size = nrow(raw_dat2[[n]]), replace=TRUE)
      temp_covmat. = data.frame("env" = raw_dat2[[n]][,"env"],
                                "phen_corrected" = c(phen_temp),
                                "gen" = raw_dat2[[n]][,"gen"])
...
new_dat <- replicate(100, gen_cov_fun(falc_dat), simplify=TRUE)
...
(cov_est_temp = cov(real_dat$G_means,real_dat$E_means))

## THESE DONT MATCH.... UGHHHHHHH WHAT IN THE WORLDDDDD
```
