...
simout <- simulateGV(0.5, 0.5, 10)
simout$covGE_true
simout$corGE_true
sampling_dist_cov <- replicate(1000, bootstrapGV(simout$data_GE), simplify=TRUE)

#rownames(sampling_dist_cov) <- c("covGE_boot", "corGE_boot")
#sampling_dist_cov <- data.frame(as.matrix(unlist(t(sampling_dist_cov))))

#cov
quantile(unlist(sampling_dist_cov[1,]), probs=c(0.025, 0.975), type=1)
...
# Do we standardize phenotype data before we put it in here?
