...
|---|---|
$PL > 1$ | countergradient with hyperplasticity |
$0 < PL < 1$ | cogradient variation |
$PL = 1$ | perfect plasticity (Figure 4 in paper) |
$PL < - 1$ | countergradient with wrong-sign plasticity |
...
The simulation results are archived on BCO-DMO:
- The `Power_output_results.csv` is available at: https://www.bco-dmo.org/dataset/877456 
- The other files are available at: 10.26008/1912/bco-dmo.934896.1  
...
TO DO: 
sum(newdf$covariance.x==0)
sum(newdf$true_cov==0)

summary(newdf$true_cov)
hist(newdf$true_cov, breaks=seq(-1,1,0.1))
hist(newdf$covariance.x, breaks=seq(-1,1,0.1))
```
